        ...
        <div style="background-color: white; min-height: 80vh;width:95%; border-radius: 12px;padding:1rem;">
            <div class="container">
                ...
                <div class="card mb-4 shadow-sm">
                    <div class="card-body">
                        <h5 class="card-title">Waste Type: {{ $wasteLog->WasteType }}</h5>
                        <p class="card-text">
                            <strong>Restaurant Owner:</strong> {{ $wasteLog->restaurantOwner->Name ?? 'N/A' }}<br>
                            <strong>Restaurant ID:</strong> {{ $wasteLog->RestaurantOwnerID ?? 'N/A' }}<br>
                            <strong>Weight:</strong> {{ $wasteLog->Weight ?? 'N/A' }} kg<br>
                            <strong>Date Logged:</strong> {{ $wasteLog->DateLogged->format('Y-m-d') ?? 'N/A' }}<br>
                            <strong>Price Per Kg:</strong> ${{ $wasteLog->priceList->price_per_kg ?? 'N/A' }}<br>
                            <strong>Subscription Prices:</strong>
                        </p>
                        <ul>
                            <li>3 Months: ${{ $wasteLog->priceList->price_per_subscription_3 ?? 'N/A' }}</li>
                            <li>6 Months: ${{ $wasteLog->priceList->price_per_subscription_6 ?? 'N/A' }}</li>
                            <li>9 Months: ${{ $wasteLog->priceList->price_per_subscription_9 ?? 'N/A' }}</li>
                            <li>12 Months: ${{ $wasteLog->priceList->price_per_subscription_12 ?? 'N/A' }}</li>
                        </ul>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <form action="{{ route('subscription.storeCPSubscribeRO') }}" method="POST">
                                ...
                                <input type="hidden" name="ProductableType" value="waste_logs">
                                <input type="hidden" name="ProductableID" value="{{ $wasteLog->id }}">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="SubscriptionType" id="SubscriptionType" class="form-select"
                                            ...
                                            <option value="3">3 Months -
                                                ${{ $wasteLog->priceList->price_per_subscription_3 }}</option>
                                            <option value="6">6 Months -
                                                ${{ $wasteLog->priceList->price_per_subscription_6 }}</option>
                                            <option value="9">9 Months -
                                                ${{ $wasteLog->priceList->price_per_subscription_9 }}</option>
                                            <option value="12">12 Months -
                                                ${{ $wasteLog->priceList->price_per_subscription_12 }}</option>
                                        </select>
                                    </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const prices = {
                        3: {{ $wasteLog->priceList->price_per_subscription_3 ?? '0' }},
                        6: {{ $wasteLog->priceList->price_per_subscription_6 ?? '0' }},
                        9: {{ $wasteLog->priceList->price_per_subscription_9 ?? '0' }},
                        12: {{ $wasteLog->priceList->price_per_subscription_12 ?? '0' }}
                    };
